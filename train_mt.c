	...
	while (epochMissAverage > ERROR){
		...
		//Back Propogate
			...
			for (int k=1; k<T; k++){
                K = k;
                for (int q=1; q<8; q++){
				    pthread_create(&p[q],NULL,mult_thread,(void*)q);
			    }

                for (int q=1; q<8; q++){
				    pthread_join(&p[q],NULL);
			    }
            }
			float sum = 0;
		...
		//write2DArrayToFile(B, "BTemp.skynet");
		//write3DArrayToFile(R, "RTemp.skynet");
		
	}		//End Training

	//Save Parameters to file
	write2DArrayToFile(B, "B.skynet");
	write3DArrayToFile(R, "R.skynet");
...
void write3DArrayToFile(double array[N][N][T+1], const char* filename){
	...
	fwrite(array, sizeof(double), N*N*(T), file);
	fclose (file);
}
...
void write2DArrayToFile(double array[N][T+1], const char* filename){
	...
	fwrite(array, sizeof(double), N * T, file);
	fclose(file);
}
...
void *mult_thread(void *q){
    //calculates 98 neurons
    int starting_neuron;
    starting_neuron = (int)q;
    starting_neuron = 98 * starting_neuron;

    uint64_t mu, nu;

 int K=0;

	for (mu=starting_neuron; mu<starting_neuron+98; mu++){
		dB[mu][T-K] = 0;
		for(int a=0; a<N; a++){
